...
class Scraper48:
    ...
    def scrape_single(self,url,data):
        ...
        parsed_data_list = parsed_data['DRAWER_DETAIL_LIST']
        print(parsed_data)
        ...
        # for i in range(len(parsed_data_list)):
            # if parsed_data_list[i]['LABEL'] == 'Debtor Name':
            #     if result['name'] == '':
            #         result['name'] = parsed_data_list[i]['VALUE']
            # elif parsed_data_list[i]['LABEL'] == 'Debtor Address':
            #     if result['addres'] == '':
            #         result['address']= parsed_data_list[i]['VALUE']
            # elif parsed_data_list[i]['LABEL'] == 'Secured Party Name':
            #     if result['secured_party_name'] == '':
            #         result['secured_party_name'] = parsed_data_list[i]['VALUE']
            # elif parsed_data_list[i]['LABEL'] == 'Secured Party Address':
            #     if result['secured_party_address'] == '':
            #         result['secured_party_address'] = parsed_data_list[i]['VALUE']
            # else:
            #     print(f"Label {parsed_data_list[i]['LABEL']} not handled.")

            # # Append to list once end of the list is reached, reset result dict
        ...
        # debtor_name = soup.find(attrs={'name':'primaryOwnerFirstName'})['value'] if soup.find(attrs={'name':'primaryOwnerFirstName'}) else None
        # debtor_ad = soup.find(attrs={'name':'primaryOwnerLastName'})['value'] if soup.find(attrs={'name':'primaryOwnerLastName'}) else None
        # secured_party_name = soup.find(attrs={'name':'primaryOwnerEmail'})['value'] if soup.find(attrs={'name':'primaryOwnerEmail'}) else None
        # secured_party_ad = soup.find(attrs={'name':'primaryOwnerMobile'})['value'] if soup.find(attrs={'name':'primaryOwnerMobile'}) else None
